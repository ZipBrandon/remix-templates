...
function _flatRoutes(appDir, ignoredFilePatternsOrOptions, options) {
    ...
    const routeRegex = getRouteRegex((_d = options.routeRegex) !== null && _d !== void 0 ? _d : defaultOptions.routeRegex, options);
    for (let routeDir of routeDirs) {
        visitFiles(path.join(appDir, routeDir), file => {
            if (ignoredFilePatterns &&
                ignoredFilePatterns.some(pattern => (0, minimatch_1.minimatch)(file, pattern, { dot: true }))) {
                return;
            }
...
function isIndexRoute(routeId, options) {
...
function getRouteInfo(routeDir, file, options) {
    ...
    let routeSegments = getRouteSegments(routeIdWithoutRoutes, index, options.paramPrefixChar, options.nestedFolderChar);
...
function getRouteSegments(name, index, paramPrefixChar = '$', nestedFolderChar = "+") {
    var _a;
    let routeSegments = [];
    let i = 0;
    let routeSegment = '';
